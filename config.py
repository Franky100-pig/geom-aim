...
# ---------------- 阵亡观战 ----------------
# 阵亡后先原地倒地（眼高落到地面、视线压低），停留 DEATH_CAM_HOLD 秒再切到队友第一视角。
# 观战期间视角完全贴合队友：位置、朝向、俯仰全跟随，鼠标不参与——
# 之前鼠标事件写的是 cam.yaw 基准值，而渲染读的是 eff_yaw，顺序错了导致一动鼠标画面就甩。
DEATH_FALL_TIME = 0.45     # 倒地动画时长（秒）
DEATH_CAM_HOLD = 1.00      # 倒地后停留多久才切到队友视角
DEATH_EYE_H = 0.10         # 倒地后眼高（站立 0.5 / 蹲 0.3）
DEATH_PITCH_PX = 90.0      # 倒地时视线压低的像素量（720p 基准，按 h/720 缩放）
